...
export default function WaitlistPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        if (data.referral_link) setServerReferralLink(data.referral_link);
        if (data.referral_code) setReferralCode(data.referral_code);
        setIsSubmitted(true);
        try {
          const statsResponse = await fetch(
            `/api/waitlist/referrals?email=${encodeURIComponent(formData.email)}`
          );
          if (statsResponse.ok) {
  ...
  return (
    <div className={styles.waitlistRoot}>
      {/* Background layers */}
      ...
      <div className={styles.topBar}>
        <Link href="/" className={styles.brand}>
          <div className={styles.brandLogoRing}>
            <Image src="/logo.png" alt="Ved AI" fill className="object-contain" />
          </div>
          <span className={styles.brandName}>Ved AI</span>
        </Link>
      ...
      {currentStep === 0 && !isSubmitted && (
        <div className={styles.introScreen}>
          <p className={styles.introEyebrow}>— Early Access</p>
          <h1 className={styles.introTitle}>
            Learning, finally <em>personal.</em>
          </h1>
          ...
          <p className={styles.introNote}>Takes less than a minute</p>
        </div>
      )}
